 ...
 * <p>
 * These are most often accessed via {@link HyperionRequestLog} or {@link HyperionCommand#getExecutionEvents()}.
 */
public enum HyperionEventType {
    EMIT(false),
    SUCCESS(true),
    FAILURE(false),
    TIMEOUT(false),
    ...
    HyperionEventType(boolean isTerminal) {
    ...
    public static HyperionEventType from(HyperionRollingNumberEvent event) {
        switch (event) {
            ...
            default:
                throw new RuntimeException("Not an event that can be converted to HyperionEventType : " + event);
        }
    }
     ...
     */
    public final static List<HyperionEventType> EXCEPTION_PRODUCING_EVENT_TYPES = new ArrayList<HyperionEventType>();

    /**
     * List of events that are terminal
     */
    public final static List<HyperionEventType> TERMINAL_EVENT_TYPES = new ArrayList<HyperionEventType>();
    ...
    static {
        ...
        for (HyperionEventType eventType: HyperionEventType.values()) {
    ...
    public enum ThreadPool {
        ...
        public static ThreadPool from(HyperionRollingNumberEvent event) {
            switch (event) {
                case THREAD_EXECUTION: return EXECUTED;
                case THREAD_POOL_REJECTED: return REJECTED;
                default:
                    throw new RuntimeException("Not an event that can be converted to HyperionEventType.ThreadPool : " + event);
            }
        }

        public static ThreadPool from(HyperionEventType eventType) {
            switch (eventType) {
    ...
    public enum Collapser {
        ...
        public static Collapser from(HyperionRollingNumberEvent event) {
            switch (event) {
                case COLLAPSER_BATCH: return BATCH_EXECUTED;
                case COLLAPSER_REQUEST_BATCHED: return ADDED_TO_BATCH;
                case RESPONSE_FROM_CACHE: return RESPONSE_FROM_CACHE;
                default:
                    throw new RuntimeException("Not an event that can be converted to HyperionEventType.Collapser : " + event);
            }
        }
    }
}
