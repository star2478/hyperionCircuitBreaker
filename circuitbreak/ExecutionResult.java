...
public class ExecutionResult {
    ...
    static {
        for (HyperionEventType eventType: HyperionEventType.EXCEPTION_PRODUCING_EVENT_TYPES) {
            EXCEPTION_PRODUCING_EVENTS.set(eventType.ordinal());
        }

        for (HyperionEventType eventType: HyperionEventType.TERMINAL_EVENT_TYPES) {
    ...
    public static class EventCounts {
        ...
        EventCounts(HyperionEventType... eventTypes) {
            BitSet newBitSet = new BitSet(NUM_EVENT_TYPES);
            int localNumEmits = 0;
            int localNumFallbackEmits = 0;
            int localNumCollapsed = 0;
            for (HyperionEventType eventType: eventTypes) {
                switch (eventType) {
                    case EMIT:
                        newBitSet.set(HyperionEventType.EMIT.ordinal());
                        localNumEmits++;
                        break;
                    case FALLBACK_EMIT:
                        newBitSet.set(HyperionEventType.FALLBACK_EMIT.ordinal());
                        localNumFallbackEmits++;
                        break;
                    case COLLAPSED:
                        newBitSet.set(HyperionEventType.COLLAPSED.ordinal());
                        localNumCollapsed++;
                        break;
                    default:
        ...
        EventCounts plus(HyperionEventType eventType) {
            return plus(eventType, 1);
        }

        EventCounts plus(HyperionEventType eventType, int count) {
        ...
        public boolean contains(HyperionEventType eventType) {
        ...
        public int getCount(HyperionEventType eventType) {
            switch (eventType) {
    ...
    public static ExecutionResult from(HyperionEventType... eventTypes) {
    ...
    private static boolean didExecutionOccur(HyperionEventType eventType) {
        switch (eventType) {
    ...
    public ExecutionResult markCollapsed(HystrixCollapserKey collapserKey, int sizeOfBatch) {
        return new ExecutionResult(eventCounts.plus(HyperionEventType.COLLAPSED, sizeOfBatch), startTimestamp, executionLatency, userThreadLatency,
                failedExecutionException, executionException, executionOccurred, isExecutedInThread, collapserKey);
    }
     ...
     */
    public ExecutionResult addEvent(HyperionEventType eventType) {
    ...
    public ExecutionResult addEvent(int executionLatency, HyperionEventType eventType) {
        if (startTimestamp >= 0 && !isResponseRejected()) {
            return new ExecutionResult(eventCounts.plus(eventType), startTimestamp, executionLatency,
                    userThreadLatency, failedExecutionException, executionException,
                    executionOccurred, isExecutedInThread, collapserKey);
    ...
    public boolean isResponseSemaphoreRejected() {
        return eventCounts.contains(HyperionEventType.SEMAPHORE_REJECTED);
    }

    public boolean isResponseThreadPoolRejected() {
        return eventCounts.contains(HyperionEventType.THREAD_POOL_REJECTED);
    }
    ...
    public List<HyperionEventType> getOrderedList() {
        List<HyperionEventType> eventList = new ArrayList<HyperionEventType>();
