...
import com.pingan.testdemo.circuitbreak.HyperionCommandMetrics;
 ...
 */
public class HealthCountsStream extends BucketedRollingCounterStream<HyperionCommandCompletion, long[], HyperionCommandMetrics.HealthCounts> {

    private static final ConcurrentMap<String, HealthCountsStream> streams = new ConcurrentHashMap<String, HealthCountsStream>();

    private static final int NUM_EVENT_TYPES = HyperionEventType.values().length;

    private static final Func2<HyperionCommandMetrics.HealthCounts, long[], HyperionCommandMetrics.HealthCounts> healthCheckAccumulator = new Func2<HyperionCommandMetrics.HealthCounts, long[], HyperionCommandMetrics.HealthCounts>() {
        @Override
        public HyperionCommandMetrics.HealthCounts call(HyperionCommandMetrics.HealthCounts healthCounts, long[] bucketEventCounts) {
//        	System.out.println(Thread.currentThread().getName() + ": ---------" + Arrays.toString(bucketEventCounts));
            return healthCounts.plus(bucketEventCounts);
        }
    };


    public static HealthCountsStream getInstance(HyperionCommandKey commandKey, HyperionCommandProperties properties) {
...
//System.out.println("wolegequ------------" + properties.metricsRollingStatisticalWindowInMilliseconds().get());
        return getInstance(commandKey, numHealthCountBuckets, healthCountBucketSizeInMs);
    }

    public static HealthCountsStream getInstance(HyperionCommandKey commandKey, int numBuckets, int bucketSizeInMs) {
...
//                	HystrixCommandKey commandKey2 = HystrixCommandKey.Factory.asKey(commandKey.name());
                    HealthCountsStream newStream = new HealthCountsStream(commandKey, numBuckets, bucketSizeInMs,
                            HyperionCommandMetrics.appendEventToBucket);
//                            HystrixCommandMetrics.appendEventToBucket);
    ...
    public static void removeByKey(HyperionCommandKey key) {
        streams.remove(key.name());
    }

    private HealthCountsStream(final HyperionCommandKey commandKey, final int numBuckets, final int bucketSizeInMs,
                               Func2<long[], HyperionCommandCompletion, long[]> reduceCommandCompletion) {
        super(HyperionCommandCompletionStream.getInstance(commandKey), numBuckets, bucketSizeInMs, reduceCommandCompletion, healthCheckAccumulator);
    }
    ...
    @Override
    HyperionCommandMetrics.HealthCounts getEmptyOutputValue() {
        return HyperionCommandMetrics.HealthCounts.empty();
    }
}
