...
public class Hyperion {
    ...
    public static void reset() {
        ...
        HyperionThreadPool.Factory.shutdown();
        _reset();
    }
    ...
    public static void reset(long time, TimeUnit unit) {
        // shutdown thread-pools
        HyperionThreadPool.Factory.shutdown(time, unit);
        _reset();
    }
    ...
    private static void _reset() {
        // clear metrics
        HyperionCommandMetrics.reset();
        HyperionThreadPoolMetrics.reset();
//        HystrixCollapserMetrics.reset();
//        // clear collapsers
//        HystrixCollapser.reset();
        // clear circuit breakers
        HyperionCircuitBreaker.Factory.reset();
        HystrixPlugins.reset();
        HystrixPropertiesFactory.reset();
        currentCommand.set(new ConcurrentStack<HyperionCommandKey>());
    }

    private static ThreadLocal<ConcurrentStack<HyperionCommandKey>> currentCommand = new ThreadLocal<ConcurrentStack<HyperionCommandKey>>() {
        @Override
        protected ConcurrentStack<HyperionCommandKey> initialValue() {
            return new ConcurrentStack<HyperionCommandKey>();
        }
    };
     ...
     */
    public static HyperionCommandKey getCurrentThreadExecutingCommand() {
     ...
     */
    /* package */static Action0 startCurrentThreadExecutingCommand(HyperionCommandKey key) {
        final ConcurrentStack<HyperionCommandKey> list = currentCommand.get();
        try {
            list.push(key);
        } catch (Exception e) {
    ...
    private static void endCurrentThreadExecutingCommand(ConcurrentStack<HyperionCommandKey> list) {
        try {
            if (!list.isEmpty()) {
                list.pop();
            }
