...
/**
 * ThreadPool used to executed {@link HystrixCommand#run()} on separate threads when configured to do so with {@link HyperionCommandProperties#executionIsolationStrategy()}.
 * <p>
 * Typically each {@link HyperionCommandGroupKey} has its own thread-pool so that any one group of commands can not starve others from being able to run.
 * <p>
 * A {@link HystrixCommand} can be configured with a thread-pool explicitly by injecting a {@link HyperionThreadPoolKey} or via the
 * {@link HyperionCommandProperties#executionIsolationThreadPoolKeyOverride()} otherwise it
 * will derive a {@link HyperionThreadPoolKey} from the injected {@link HyperionCommandGroupKey}.
 * <p>
 ...
 */
public interface HyperionThreadPool {
...
//    public Scheduler getScheduler(Func0<Boolean> shouldInterruptThread);
    ...
    /* package */static class Factory {
         ...
         */
        /* package */final static ConcurrentHashMap<String, HyperionThreadPool> threadPools = new ConcurrentHashMap<String, HyperionThreadPool>();

        /**
         * Get the {@link HyperionThreadPool} instance for a given {@link HyperionThreadPoolKey}.
         * <p>
         * This is thread-safe and ensures only 1 {@link HyperionThreadPool} per {@link HyperionThreadPoolKey}.
         *
         * @return {@link HyperionThreadPool} instance
         */
        /* package */static HyperionThreadPool getInstance(HyperionThreadPoolKey threadPoolKey, HyperionThreadPoolProperties.Setter propertiesBuilder) {
            // get the key to use instead of using the object itself so that if people forget to implement equals/hashcode things will still work
            String key = threadPoolKey.name();

            // this should find it for all but the first time
            HyperionThreadPool previouslyCached = threadPools.get(key);
            if (previouslyCached != null) {
                return previouslyCached;
            }

            // if we get here this is the first time so we need to initialize
            synchronized (HyperionThreadPool.class) {
                if (!threadPools.containsKey(key)) {
                    threadPools.put(key, new HyperionThreadPoolDefault(threadPoolKey, propertiesBuilder));
                }
            }
        ...
        /**
         * Initiate the shutdown of all {@link HyperionThreadPool} instances.
         * <p>
        ...
        /* package */static synchronized void shutdown() {
            for (HyperionThreadPool pool : threadPools.values()) {
        ...
        /**
         * Initiate the shutdown of all {@link HyperionThreadPool} instances and wait up to the given time on each pool to complete.
         * <p>
         * NOTE: This is NOT thread-safe if HystrixCommands are concurrently being executed
         * and causing thread-pools to initialize while also trying to shutdown.
         * </p>
     ...
     * @ThreadSafe
     */
    /* package */static class HyperionThreadPoolDefault implements HyperionThreadPool {
        private static final Logger logger = LoggerFactory.getLogger(HyperionThreadPoolDefault.class);

        private final HyperionThreadPoolProperties properties;
//        private final BlockingQueue<Runnable> queue;
        private final ThreadPoolExecutor threadPool;
        private final HyperionThreadPoolMetrics metrics;
        private final int queueSize;

        public HyperionThreadPoolDefault(HyperionThreadPoolKey threadPoolKey, HyperionThreadPoolProperties.Setter propertiesDefaults) {
//            this.properties = HyperionPropertiesFactory.getThreadPoolProperties(threadPoolKey, propertiesDefaults);
//            HystrixConcurrencyStrategy concurrencyStrategy = HystrixPlugins.getInstance().getConcurrencyStrategy();
//            this.queueSize = properties.maxQueueSize().get();
//
//            this.metrics = HyperionThreadPoolMetrics.getInstance(threadPoolKey,
...
//        }
         ...
         *
         * Note that the <code>queueSize</code> is an final instance variable on HyperionThreadPoolDefault, and not looked up dynamically.
