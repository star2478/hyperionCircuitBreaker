...
import com.pingan.testdemo.circuitbreak.strategy.properties.HyperionProperty;
 ...
 * @ThreadSafe
 */
public class HyperionRollingNumber {
     ...
     */
    @Deprecated
    public HyperionRollingNumber(HyperionProperty<Integer> timeInMilliseconds, HyperionProperty<Integer> numberOfBuckets) {
        this(timeInMilliseconds.get(), numberOfBuckets.get());
    }

    public HyperionRollingNumber(int timeInMilliseconds, int numberOfBuckets) {
        this(ACTUAL_TIME, timeInMilliseconds, numberOfBuckets);
    }

    /* package for testing */ HyperionRollingNumber(Time time, int timeInMilliseconds, int numberOfBuckets) {
        this.time = time;
    ...
    /**
     * Increment the counter in the current bucket by one for the given {@link HyperionRollingNumberEvent} type.
     * <p>
     * The {@link HyperionRollingNumberEvent} must be a "counter" type <code>HyperionRollingNumberEvent.isCounter() == true</code>.
     * 
     * @param type
     *            HyperionRollingNumberEvent defining which counter to increment
     */
//    public void increment(HyperionRollingNumberEvent type) {
//        getCurrentBucket().getAdder(type).increment();
//    }

    /**
     * Add to the counter in the current bucket for the given {@link HyperionRollingNumberEvent} type.
     * <p>
     * The {@link HyperionRollingNumberEvent} must be a "counter" type <code>HyperionRollingNumberEvent.isCounter() == true</code>.
     * 
     * @param type
     *            HyperionRollingNumberEvent defining which counter to add to
     * @param value
     *            long value to be added to the current bucket
     */
//    public void add(HyperionRollingNumberEvent type, long value) {
...
//    }
     ...
     * <p>
     * The {@link HyperionRollingNumberEvent} must be a "max updater" type <code>HyperionRollingNumberEvent.isMaxUpdater() == true</code>.
     * 
     * @param type  HyperionRollingNumberEvent defining which counter to retrieve values from
     * @param value long value to be given to the max updater
     */
//    public void updateRollingMax(HyperionRollingNumberEvent type, long value) {
...
//            cumulativeSum.addBucket(lastBucket);
    ...
    /**
     * Get the cumulative sum of all buckets ever since the JVM started without rolling for the given {@link HyperionRollingNumberEvent} type.
     * <p>
     * See {@link #getRollingSum(HyperionRollingNumberEvent)} for the rolling sum.
     * <p>
     * The {@link HyperionRollingNumberEvent} must be a "counter" type <code>HyperionRollingNumberEvent.isCounter() == true</code>.
     * 
     * @param type HyperionRollingNumberEvent defining which counter to retrieve values from
     * @return cumulative sum of all increments and adds for the given {@link HyperionRollingNumberEvent} counter type
     */
//    public long getCumulativeSum(HyperionRollingNumberEvent type) {
...
//    }
    ...
    /**
     * Get the sum of all buckets in the rolling counter for the given {@link HyperionRollingNumberEvent} type.
     * <p>
     * The {@link HyperionRollingNumberEvent} must be a "counter" type <code>HyperionRollingNumberEvent.isCounter() == true</code>.
     * 
     * @param type
     *            HyperionRollingNumberEvent defining which counter to retrieve values from
     * @return
     *         value from the given {@link HyperionRollingNumberEvent} counter type
     */
//    public long getRollingSum(HyperionRollingNumberEvent type) {
...
//    }
    ...
    /**
     * Get the value of the latest (current) bucket in the rolling counter for the given {@link HyperionRollingNumberEvent} type.
     * <p>
     * The {@link HyperionRollingNumberEvent} must be a "counter" type <code>HyperionRollingNumberEvent.isCounter() == true</code>.
     * 
     * @param type
     *            HyperionRollingNumberEvent defining which counter to retrieve value from
     * @return
     *         value from latest bucket for given {@link HyperionRollingNumberEvent} counter type
     */
//    public long getValueOfLatestBucket(HyperionRollingNumberEvent type) {
//        Bucket lastBucket = getCurrentBucket();
//        if (lastBucket == null)
//            return 0;
...
//    }
    ...
    /**
     * Get an array of values for all buckets in the rolling counter for the given {@link HyperionRollingNumberEvent} type.
     * <p>
     ...
     * @param type
     *            HyperionRollingNumberEvent defining which counter to retrieve values from
     * @return array of values from each of the rolling buckets for given {@link HyperionRollingNumberEvent} counter type
     */
//    public long[] getValues(HyperionRollingNumberEvent type) {
//        Bucket lastBucket = getCurrentBucket();
//        if (lastBucket == null)
//            return new long[0];
//
...
//    }
    ...
    /**
     * Get the max value of values in all buckets for the given {@link HyperionRollingNumberEvent} type.
     * <p>
     * The {@link HyperionRollingNumberEvent} must be a "max updater" type <code>HyperionRollingNumberEvent.isMaxUpdater() == true</code>.
     * 
     * @param type
     *            HyperionRollingNumberEvent defining which "max updater" to retrieve values from
     * @return max value for given {@link HyperionRollingNumberEvent} type during rolling window
     */
//    public long getRollingMaxValue(HyperionRollingNumberEvent type) {
...
//             * of all types for each so we only allocate the objects if the HyperionRollingNumberEvent matches
...
//            adderForCounterType = new LongAdder[HyperionRollingNumberEvent.values().length];
//            for (HyperionRollingNumberEvent type : HyperionRollingNumberEvent.values()) {
//                if (type.isCounter()) {
//                    adderForCounterType[type.ordinal()] = new LongAdder();
//                }
//            }
//
//            updaterForCounterType = new LongMaxUpdater[HyperionRollingNumberEvent.values().length];
//            for (HyperionRollingNumberEvent type : HyperionRollingNumberEvent.values()) {
...
//        }
//
//        long get(HyperionRollingNumberEvent type) {
//            if (type.isCounter()) {
...
//        }
//
//        LongAdder getAdder(HyperionRollingNumberEvent type) {
//            if (!type.isCounter()) {
...
//        }
//
//        LongMaxUpdater getMaxUpdater(HyperionRollingNumberEvent type) {
//            if (!type.isMaxUpdater()) {
